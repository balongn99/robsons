#!/usr/bin/env python3
"""
run_one_task.py  —  optimise a single Robson-cage molecule

• Scans every physically allowed multiplicity.
• Keeps ONE “best” record using this rule:
      – if nothing stored yet       → accept
      – if new result converged and current best not   → accept
      – if both share convergence status               → accept lower-E
• Writes
      <RUN_DIR>/<tag>.traj              – relaxed geometry (best)
      <RUN_DIR>/<tag>_m<M>.traj         – trajectory per multiplicity
      <RUN_DIR>/<tag>.ok | .fail        – sentinel
      <RUN_DIR>/results/<tag>.json      – metadata (only if converged)
• Uses a *.lock* file so two processes can’t touch the same tag.
"""
...
def load_spin_data(mod="Spin") -> Dict[Tuple[str, int], List[float]]:
    ...
    if hasattr(m, "df"):
        ...
        return {(r.Element, int(r[col])): _as_list(r.Spins) for _, r in df.iterrows()}
...
def worker(task):
    ...
    try:
        ...
        for mult in sorted({int(2*S + 1) for S in total_S_set(S1_ref, S2_ref)}):
            ...
            E, conv = trial.get_potential_energy(), opt.converged()
            better = (
                best_E is None or
        ...
        if best_mol is None:        # total failure
            return None

        write(RUN_DIR / f"{tag}.traj", best_mol)

        if best_conv:
            (RUN_DIR / f"{tag}.ok").touch()
